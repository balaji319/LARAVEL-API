...
class InboundIvr extends React.Component {
  ...
  componentDidMount() {
    ...
    axios
      .get("api/inbound_calls_report_groups_list/", requestOptions)
      .then(response => {
        this.setState({
  ...
  render() {
    ...
    if (flag) {
      ...
      counts1.slice(0).sort(function(a, b) {
        return a - b;
      });
          ...
          if (mydata.timestat[i][1] > 0) {
            for (j = 0; j < counts1.length; j++) {
              if (counts1[j].key == mydata.timestat[i][1].trim()) {
                ...
                } else key_co = mydata.timestat[i][1].trim();
              }
            }
            ...
            if (last_key == 0) var remain1 = count - (last_key + 1);
            else var remain1 = count - last_key;
            if (mydata.timestat[i][1]) {
              my_data.push({
                first: mydata.timestat[i][0],
                data: mydata.timestat[i][1],
                last_key: last_key,
                remain1: remain1,
                count: count
              });
    ...
    return (
      <div>
        ...
        <Card className="shadow border-0 bg-default text-black">
          <CardBody>
            <h4>
              IVR Stats Report {from + "  To  " + to} Date:{" "}
              {momentTimezone
                .tz(moment(new Date()), "America/Los_Angeles")
            ...
            {flag == false ? (
              <div>
                ...
                <h4>TIME STATS</h4>
                <h4>
                  GRAPH IN 15 MINUTE INCREMENTS OF TOTAL CALLS TAKEN INTO THIS
                  IVR
                </h4>
                <Paper>
                  <div className="table-responsive-material">
              ...
              <div>
                      ...
                      <Table>
                        ...
                        <TableBody>
                          {Object.keys(mydata.total_ivr).map((key, val) => (
                            <TableRow key={val}>
                              <TableCell>
                                {mydata.total_ivr[key].ivrcalls}
                              </TableCell>
                              <TableCell>
                                {mydata.total_ivr[key].queuecalls}
                              </TableCell>
                              <TableCell>
                                {mydata.total_ivr[key].queuedrop}
                              </TableCell>
                              <TableCell>
                                {mydata.total_ivr[key].dropcallpercent}%
                              </TableCell>
                              <TableCell>
                                {mydata.total_ivr[key].ivravgtime}
                              </TableCell>
                              <TableCell>
                                {mydata.total_ivr[key].totalavgtime}
                              </TableCell>
                              <TableCell>
                                {mydata.total_ivr[key].callstat}
                              </TableCell>
                            </TableRow>
                          ))}
                          <TableRow>
                    ...
                    <div className="table-responsive-material">
                      <Table>
                        <TableHead>
                          <TableRow>
                            <TableCell
                              onClick={() => this.clickCol(1, "IVR CALLS")}
                            >
                              <span className="ancr-style">IVR CALLS</span>
                            </TableCell>
                            <TableCell
                              onClick={() => this.clickCol(2, "QUEUE CALLS")}
                            >
                              <span className="ancr-style">QUEUE CALLS</span>
                            </TableCell>
                            <TableCell
                              onClick={() =>
                                this.clickCol(3, "QUEUE DROP CALLS")
                              }
                            >
                              <span className="ancr-style">
                                QUEUE DROP CALLS
                              </span>
                            </TableCell>
                            <TableCell
                              onClick={() =>
                                this.clickCol(4, "QUEUE DROP PERCENTAGE")
                              }
                            >
                              <span className="ancr-style">
                                QUEUE DROP PERCENTAGE
                              </span>
                            </TableCell>
                            <TableCell
                              onClick={() =>
                                this.clickCol(5, "IVR AVERAGE TIME")
                              }
                            >
                              <span className="ancr-style">
                                IVR AVERAGE TIME
                            ...
                            <TableCell
                              onClick={() =>
                                this.clickCol(6, "TOTAL AVERAGE TIME")
                              }
                            >
                              <span className="ancr-style">
                                TOTAL AVERAGE TIME
                        ...
                        <TableBody>
                          {Object.keys(mydata.total_ivr).map((key, val) => (
                            <TableRow key={val}>
                              <TableCell
                                className={
                                  activeCol != 0 && activeCol != 1
                                    ? "hide-td"
                                    : ""
                                }
                                colSpan={activeCol == 1 ? 3 : 1}
                              >
                                <Progress
                                  percent={mydata.total_ivr[key].ivrcalls}
                                />
                              </TableCell>
                              <TableCell
                                className={
                                  activeCol != 0 && activeCol != 2
                                    ? "hide-td"
                                    : ""
                                }
                                colSpan={activeCol == 2 ? 3 : 1}
                              >
                                <Progress
                                  percent={mydata.total_ivr[key].queuecalls}
                                />
                              </TableCell>
                              <TableCell
                                className={
                                  activeCol != 0 && activeCol != 3
                                    ? "hide-td"
                                    : ""
                                }
                                colSpan={activeCol == 3 ? 3 : 1}
                              >
                                <Progress
                                  percent={mydata.total_ivr[key].queuedrop}
                                />
                              </TableCell>
                              <TableCell
                                className={
                              ...
                              <TableCell
                                className={
                                  activeCol != 0 && activeCol != 5
                                    ? "hide-td"
                                    : ""
                                }
                                colSpan={activeCol == 5 ? 3 : 1}
                              >
                                <Progress
                                  percent={mydata.total_ivr[key].ivravgtime}
                                />
                              </TableCell>
                              <TableCell
                                className={
                                  activeCol != 0 && activeCol != 6
                                    ? "hide-td"
                                    : ""
                                }
                                colSpan={activeCol == 6 ? 3 : 1}
                              >
                                <Progress
                                  percent={mydata.total_ivr[key].totalavgtime}
                                />
                              </TableCell>
                              <TableCell colSpan={3}>
                                {mydata.total_ivr[key].callstat}
                ...
                <div className="row">
                  <div className="col-lg-12 col-sm-12 col-12">
                    <label>TIME STATS</label>
                    <br />
                    <label>
                      GRAPH IN 15 MINUTE INCREMENTS OF TOTAL CALLS TAKEN INTO
                      THIS IVR
                    </label>
                  </div>
                </div>
                              ...
                              my_data[key].data ? (
                                [
                                  <TableCell
                                    ...
                                    colSpan={my_data[key].last_key}
                                  />,
                                  <TableCell
                                    key={Math.random()}
                                    colSpan={my_data[key].remain1}
                                  />
                                ]
                              ) : (
                                [
                                  (my_data[key].count = my_data[key]
                                    .last_key ? (
                                    [
                                      <TableCell
                                        key={Math.random()}
                                    ...
                                    <TableCell colSpan={my_data[key].count} />
                                  ))
                                ]
                              )
                            ) : (
                              <TableCell colSpan={mydata.callscale.length} />
                            )}
                            <TableCell>{my_data[key].data}</TableCell>
                          </TableRow>
                        ))}
                        <TableRow>
                          <TableCell colSpan={mydata.callscale.length + 1} />
                          <TableCell>{mydata.total_calls}</TableCell>
                        </TableRow>
                      </TableBody>
                    </Table>
