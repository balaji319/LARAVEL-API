...
class OutboundSummaryIntervals extends React.Component {
  ...
  handleSubmit = () => {
    ...
    if (status == false) {
      ...
      axios
        .post(
          "api/outbound_summary_interval_report/",
          {
            startdate: from,
            enddate: to,
            shift: selected_time,
  ...
  componentDidMount() {
    ...
    axios
      .get("api/agent_campaign_option_list/", requestOptions)
      .then(response => {
        this.setState({
          campaigns: response.data.data,
          selected_campaign: this.state.selected_campaign.concat([
            response.data.data[1].campaign_id
          ]),
          flag: true
        });
        this.handleSubmit();
    ...
    axios
      .get("api/inbound-call-time/", requestOptions)
      .then(response => {
        this.setState({
  ...
  render() {
    ...
    if (isShow) {
      data.campaign_wise_details.map((val, key) => {
        totalfirstTable1 =
          totalfirstTable1 + data.campaign_wise_details[key].totalcallevents;
        totalfirstTable2 =
          totalfirstTable2 + data.campaign_wise_details[key].totalsystemcalls;
        totalfirstTable3 =
          totalfirstTable3 + data.campaign_wise_details[key].totalagentscalls;
        totalfirstTable4 =
          totalfirstTable4 +
          data.campaign_wise_details[key].totalsalecallsarray;
        totalfirstTable5 =
          totalfirstTable5 + data.campaign_wise_details[key].totaldnccallsarray;
        totalfirstTable6 =
          totalfirstTable6 + data.campaign_wise_details[key].gna_percent;
        totalfirstTable7 =
          totalfirstTable7 + data.campaign_wise_details[key].gdroppercent;
      });
    }
    ...
    return (
      <div>
        ...
        <ContainerHeader
          ...
          title="Outbound Summary Interval Report"
        />

        <div className="row">
          <CardBox
            styleName="col-lg-12"
            heading="Outbound Summary Interval Report"
          >
                  ...
                  <FormControl className="w-100 mb-2">
                    ...
                    >
                      {Object.keys(call_time).map((item, i) => (
                        <MenuItem key={i} value={call_time[item].call_time_id}>
                          {call_time[item].call_time_id} -{" "}
                          {call_time[item].call_time_name}
                        </MenuItem>
                      ))}
                      ;
                    </Select>
                  ...
                  <Table>
                    <TableHead>
                      ...
                      <TableRow>
                        ...
                        <TableCell
                          onClick={() => this.clickCol(1, "TOTAL CALLS")}
                        >
                          TOTAL CALLS{" "}
                        </TableCell>
                        <TableCell
                          onClick={() =>
                            this.clickCol(2, "SYSTEM RELEASE CALLS")
                          }
                        >
                        ...
                        <TableCell
                          onClick={() =>
                            this.clickCol(3, "AGENT RELEASE CALLS")
                          }
                        >
                          AGENT RELEASE CALLS{" "}
                        </TableCell>
                        <TableCell
                          onClick={() => this.clickCol(4, "SALE CALLS")}
                        >
                          SALE CALLS{" "}
                        </TableCell>
                        <TableCell
                          onClick={() => this.clickCol(5, "DNC CALLS")}
                        >
                          DNC CALLS{" "}
                        </TableCell>
                        <TableCell
                          onClick={() => this.clickCol(6, "NO ANSWER PERCENT")}
                        >
                          NO ANSWER PERCENT{" "}
                        </TableCell>
                        <TableCell
                          onClick={() => this.clickCol(7, "DROP PERCENT")}
                        >
                          DROP PERCENT{" "}
                        </TableCell>
                        <TableCell
                          onClick={() =>
                            this.clickCol(8, "AGENT LOGIN TIME(H:M:S)")
                          }
                        >
                        ...
                        <TableCell
                          onClick={() =>
                            this.clickCol(9, "AGENT PAUSE TIME(H:M:S)")
                          }
                        >
                    ...
                    <TableBody>
                      {data.campaign_wise_details.map((val, key) => (
                        <TableRow key={key}>
                          <TableCell>
                            {data.campaign_wise_details[key].campaign_id +
                              " - " +
                              data.campaign_wise_details[key].campaign_name
                                .campaign_name}{" "}
                          </TableCell>
                          <TableCell
                            className={
                              activeCol != 0 && activeCol != 1 ? "hide-td" : ""
                            }
                            colSpan={activeCol == 1 ? BREAKDOWN_COL : 1}
                          >
                            {data.campaign_wise_details[key].totalcallevents}
                          </TableCell>
                          <TableCell
                            className={
                              activeCol != 0 && activeCol != 2 ? "hide-td" : ""
                            }
                            colSpan={activeCol == 2 ? BREAKDOWN_COL : 1}
                          >
                            {data.campaign_wise_details[key].totalsystemcalls}
                          </TableCell>
                          <TableCell
                            className={
                              activeCol != 0 && activeCol != 3 ? "hide-td" : ""
                            }
                            colSpan={activeCol == 3 ? BREAKDOWN_COL : 1}
                          >
                            {data.campaign_wise_details[key].totalagentscalls}
                          </TableCell>
                          <TableCell
                            className={
                              activeCol != 0 && activeCol != 4 ? "hide-td" : ""
                            }
                            colSpan={activeCol == 4 ? BREAKDOWN_COL : 1}
                          >
                            {
                              data.campaign_wise_details[key]
                                .totalsalecallsarray
                            }
                          </TableCell>
                          <TableCell
                            className={
                              activeCol != 0 && activeCol != 5 ? "hide-td" : ""
                            }
                            colSpan={activeCol == 5 ? BREAKDOWN_COL : 1}
                          >
                            {data.campaign_wise_details[key].totaldnccallsarray}
                          </TableCell>
                          <TableCell
                            className={
                              activeCol != 0 && activeCol != 6 ? "hide-td" : ""
                            }
                            colSpan={activeCol == 6 ? BREAKDOWN_COL : 1}
                          >
                            {data.campaign_wise_details[key].gna_percent}%
                          </TableCell>
                          <TableCell
                            className={
                              activeCol != 0 && activeCol != 7 ? "hide-td" : ""
                            }
                            colSpan={activeCol == 7 ? BREAKDOWN_COL : 1}
                          >
                            {data.campaign_wise_details[key].gdroppercent}%
                          </TableCell>
                          <TableCell
                            className={
                              activeCol != 0 && activeCol != 8 ? "hide-td" : ""
                            }
                            colSpan={activeCol == 8 ? BREAKDOWN_COL : 1}
                          >
                            {data.campaign_wise_details[key].loginagent_sec}
                          </TableCell>
                          <TableCell
                            className={
                              activeCol != 0 && activeCol != 9 ? "hide-td" : ""
                            }
                            colSpan={activeCol == 9 ? BREAKDOWN_COL : 1}
                          >
                        ...
                        <Table>
                          ...
                          <TableBody>
                            {data.sub_graph_stats[key].map((v, k) => (
                              <TableRow key={k}>
                                <TableCell>
                                  {data.sub_graph_stats[key][k][0]}
                                </TableCell>
                                <TableCell>
                                  {data.sub_graph_stats[key][k][1]}
                                </TableCell>
                                <TableCell>
                                  {data.sub_graph_stats[key][k][2]}
                                </TableCell>
                                <TableCell>
                                  {data.sub_graph_stats[key][k][3]}
                                </TableCell>
                                <TableCell>
                                  {data.sub_graph_stats[key][k][4]}
                                </TableCell>
                                <TableCell>
                                  {data.sub_graph_stats[key][k][5]}
                                </TableCell>
                                <TableCell>
                                  {data.sub_graph_stats[key][k][6]}%
                                </TableCell>
                                <TableCell>
                                  {data.sub_graph_stats[key][k][7]}%
                                </TableCell>
                                <TableCell>
                                  {data.sub_graph_stats[key][k][10]}
                                </TableCell>
                                <TableCell>
                                  {data.sub_graph_stats[key][k][11]}
                                </TableCell>
                              </TableRow>
                            ))}
